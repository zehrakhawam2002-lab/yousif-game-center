...
export default function ReportsPage() {
  ...
  const loadData = async (p) => {
    ...
    setStats({
      totalIncome: gamingIncome + ordersIncome,
      gamingIncome,
      ordersIncome,
      totalHours,
    ...
    const catMap = { ps: 0, billiards: 0, other: 0, orders: ordersIncome }
    ;(sessions || []).forEach(s => {
      if (s.stations?.type) catMap[s.stations.type] = (catMap[s.stations.type] || 0) + (s.total || 0)
    })
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px', marginBottom: '1.5rem' }}>
                {[
                  ...
                  { icon: '🎮', label: 'Gaming Income', value: fmtIQD(stats.gamingIncome) },
                  { icon: '🧾', label: 'Orders Income', value: fmtIQD(stats.ordersIncome) },
                  { icon: '⏱️', label: 'Total Hours', value: stats.totalHours.toFixed(1) + 'h' },
                  ...
                  <div key={s.label} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '1.1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '20px', fontWeight: '700', color: '#1e293b' }}>{s.value}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', overflow: 'hidden' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                    ...
                    <tbody>
                      {byDevice.length ? byDevice.map(d => (
                        <tr key={d.name}>
                          <td style={tdStyle}><strong>{d.name}</strong><br/><span style={{ fontSize: '10px', background: '#eef2ff', color: '#6366f1', padding: '1px 7px', borderRadius: '10px', fontWeight: '700' }}>{d.type?.toUpperCase()}</span></td>
                          <td style={tdStyle}>{d.hours.toFixed(1)}h</td>
                          <td style={tdStyle}>{d.sessions}</td>
                          <td style={{ ...tdStyle, fontWeight: '700', color: '#6366f1' }}>{fmtIQD(d.income)}</td>
                        </tr>
                      )) : <tr><td colSpan={4} style={{ textAlign: 'center', color: '#94a3b8', padding: '2rem' }}>No data yet</td></tr>}
                    </tbody>
                  </table>
                </div>
                    ...
                    <tbody>
                      {byCategory.map(c => (
                        <tr key={c.label}>
                          ...
                          <td style={{ ...tdStyle, fontWeight: '700' }}>{fmtIQD(c.income)}</td>
                          <td style={tdStyle}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <div style={{ flex: 1, height: '6px', background: '#f0f2f5', borderRadius: '3px' }}>
                                <div style={{ width: `${c.pct}%`, height: '100%', background: '#6366f1', borderRadius: '3px' }}></div>
                              </div>
                              <span style={{ fontSize: '12px', fontWeight: '600', minWidth: '30px' }}>{c.pct}%</span>
                            </div>
                          </td>
                        </tr>
                      ))}
