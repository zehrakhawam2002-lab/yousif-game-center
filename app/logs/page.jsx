                ...
                <tbody>
                  {sessions.length ? sessions.map(s => (
                    <tr key={s.id} style={{ cursor: 'default' }}>
                      <td style={tdStyle}><div style={{ fontSize: '12px', fontWeight: '600' }}>{fmtTime(s.ended_at)}</div><div style={{ fontSize: '11px', color: '#94a3b8' }}>{fmtDate(s.ended_at)}</div></td>
                      <td style={tdStyle}><strong>{s.stations?.name}</strong><br/><span style={{ fontSize: '10px', background: '#eef2ff', color: '#6366f1', padding: '1px 7px', borderRadius: '10px', fontWeight: '700' }}>{s.stations?.type?.toUpperCase()}</span></td>
                      <td style={tdStyle}>{s.customer_name}</td>
                      <td style={tdStyle}>{fmtDur(s.duration_minutes)}</td>
                      <td style={tdStyle}>{fmtIQD(s.gaming_cost || 0)}</td>
                      <td style={tdStyle}>{fmtIQD(s.orders_total || 0)}</td>
                      <td style={tdStyle}>{s.discount_percent ? <span style={{ background: '#fef3c7', color: '#92400e', padding: '2px 8px', borderRadius: '10px', fontSize: '11px', fontWeight: '700' }}>{s.discount_percent}%</span> : '—'}</td>
                      <td style={{ ...tdStyle, fontWeight: '700', color: '#6366f1' }}>{fmtIQD(s.total || 0)}</td>
                    </tr>
                  )) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                ...
                <tbody>
                  {orders.length ? orders.map(o => (
                    <tr key={o.id}>
                      <td style={tdStyle}><div style={{ fontSize: '12px', fontWeight: '600' }}>{fmtTime(o.created_at)}</div><div style={{ fontSize: '11px', color: '#94a3b8' }}>{fmtDate(o.created_at)}</div></td>
                      <td style={tdStyle}>{(o.order_items || []).map(i => `${i.name} ×${i.quantity}`).join(', ') || '—'}</td>
                      <td style={tdStyle}>{fmtIQD(o.subtotal || 0)}</td>
                      <td style={tdStyle}>{o.discount_percent ? <span style={{ background: '#fef3c7', color: '#92400e', padding: '2px 8px', borderRadius: '10px', fontSize: '11px', fontWeight: '700' }}>{o.discount_percent}%</span> : '—'}</td>
                      <td style={{ ...tdStyle, fontWeight: '700', color: '#6366f1' }}>{fmtIQD(o.total || 0)}</td>
                    </tr>
                  )) : (
