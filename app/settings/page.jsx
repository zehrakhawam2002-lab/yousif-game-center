...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({ center_name: '', center_code: '', branch_name: '', currency: 'IQD', price_ps: 3000, price_pc: 2500, price_billiards_single: 4000, price_billiards_double: 6000, price_table_tennis: 3000, price_foosball: 3000, price_chips: 3000 })
  ...
  const saveSettings = async () => {
    await supabase.from('settings').update({
      center_name: form.center_name, center_code: form.center_code, branch_name: form.branch_name, currency: form.currency,
      price_ps: parseInt(form.price_ps), price_pc: parseInt(form.price_pc),
      price_billiards_single: parseInt(form.price_billiards_single), price_billiards_double: parseInt(form.price_billiards_double),
      price_table_tennis: parseInt(form.price_table_tennis), price_foosball: parseInt(form.price_foosball), price_chips: parseInt(form.price_chips),
  ...
  const savePassword = async () => {
    ...
    setPassForm({ newPass: '', confirmPass: '' }); setPassSaved(true); setTimeout(() => setPassSaved(false), 3000)
  }
  ...
  const PriceRow = ({ label, sub, valueKey }) => (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', marginBottom: '8px' }}>
      <div><div style={{ fontSize: '13px', fontWeight: '600', color: '#1e293b' }}>{label}</div><div style={{ fontSize: '11px', color: '#64748b' }}>{sub}</div></div>
        ...
        <div style={{ padding: '1.5rem', maxWidth: '800px' }}>
          ...
          <Section title="🏢 Center Information">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 12px' }}>
              <Field label="Center Name" value={form.center_name || ''} onChange={e => setForm({...form, center_name: e.target.value})} placeholder="Novixiq Game Center System" />
              <Field label="Center Code" value={form.center_code || ''} onChange={e => setForm({...form, center_code: e.target.value})} placeholder="NGC-2024" />
          ...
          <Section title="💰 Prices per Hour (IQD)">
            ...
            <PriceRow label="🏓 Table Tennis" sub="Per hour" valueKey="price_table_tennis" />
