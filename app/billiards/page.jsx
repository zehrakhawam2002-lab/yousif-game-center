...
const NAV = [
  ...
  { icon: '🎱', label: 'Billiards', path: '/billiards', active: true },
...
export default function BilliardsPage() {
  ...
  const [settings, setSettings] = useState({ price_billiards_single: 4000, price_billiards_double: 6000, currency: 'IQD' })
  ...
  const loadData = async () => {
    const { data: st } = await supabase.from('stations').select('*, sessions(id, started_at, customer_name, status)').eq('type', 'billiards').eq('is_active', true).order('created_at')
  ...
  const getPrice = st => st.mode === 'double' ? settings.price_billiards_double : settings.price_billiards_single
  ...
  const endSession = async () => {
    ...
    const gc = (elapsed / 3600000) * price, it = endItems.reduce((a, i) => a + i.price * i.quantity, 0)
  ...
  const addStation = async () => {
    if (!newName) return
    await supabase.from('stations').insert({ name: newName, type: 'billiards', mode: newMode })
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Segoe UI', system-ui, sans-serif", background: '#f0f2f5' }}>
      ...
      <div style={{ marginLeft: '235px', flex: 1 }}>
        ...
        <div style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1e293b' }}>Billiards 🎱</h2>
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button onClick={() => setShowAddModal(true)} style={{ background: '#6366f1', color: '#fff', border: 'none', borderRadius: '8px', padding: '6px 14px', fontSize: '13px', cursor: 'pointer', fontWeight: '600' }}>+ Add Table</button>
            </div>
          </div>
                ...
                <div key={st.id} style={{ background: '#fff', border: session ? '2px solid #10b981' : '1px solid #e2e8f0', borderRadius: '12px', padding: '1.1rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '10px', color: '#64748b', background: '#f0f2f5', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '2px 8px', display: 'inline-block', marginTop: '2px' }}>{st.mode === 'double' ? 'Double 2P' : 'Single 1P'}</div>
                    </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '400px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <span style={{ fontSize: '15px', fontWeight: '700', color: '#1e293b' }}>Add Billiards Table</span>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '5px', display: 'block', textTransform: 'uppercase' }}>Table Name</label>
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. Table #3" style={{ width: '100%', padding: '9px 11px', border: '1px solid #cbd5e1', borderRadius: '9px', fontSize: '13px', boxSizing: 'border-box' }} />
            </div>
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '5px', display: 'block', textTransform: 'uppercase' }}>Mode</label>
              <select value={newMode} onChange={e => setNewMode(e.target.value)} style={{ width: '100%', padding: '9px 11px', border: '1px solid #cbd5e1', borderRadius: '9px', fontSize: '13px' }}>
                <option value="single">Single — {fmtIQD(settings.price_billiards_single)}/hr</option>
                <option value="double">Double — {fmtIQD(settings.price_billiards_double)}/hr</option>
              </select>
            </div>
      ...
      {showEndModal && (() => {
        ...
        const gc = (elapsed/3600000)*price, it = endItems.reduce((a,i)=>a+i.price*i.quantity,0), da = gc*(discount/100), total = gc-da+it
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
