...
export default function StationsPage() {
  ...
  const loadData = async () => {
    const { data: st } = await supabase.from('stations').select('*, sessions(id, started_at, customer_name, status)').eq('type', 'ps').eq('is_active', true).order('created_at')
  ...
  const deleteStation = async (id) => { if (!confirm('Delete?')) return; await supabase.from('stations').update({is_active:false}).eq('id',id); loadData() }
      ...
      <div style={{ marginLeft: '235px', flex: 1 }}>
        ...
        <div style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1e293b' }}>PS Stations 🎮</h2>
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button onClick={() => setShowAddModal(true)} style={{ background: '#6366f1', color: '#fff', border: 'none', borderRadius: '8px', padding: '6px 14px', fontSize: '13px', cursor: 'pointer', fontWeight: '600' }}>+ Add Station</button>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '12px' }}>
            {stations.map(st => {
              const session=getActiveSession(st), elapsed=getElapsed(session), cost=session?(elapsed/3600000)*settings.price_ps:0
              return (
                <div key={st.id} style={{ background: '#fff', border: session?'2px solid #10b981':'1px solid #e2e8f0', borderRadius: '12px', padding: '1.1rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                    <div><div style={{ fontSize: '14px', fontWeight: '700', color: '#1e293b' }}>{st.name}</div><div style={{ fontSize: '10px', color: '#64748b', background: '#f0f2f5', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '2px 8px', display: 'inline-block', marginTop: '2px' }}>PS</div></div>
                    {session?<div style={{ fontSize: '10px', fontWeight: '700', color: '#065f46', background: '#d1fae5', borderRadius: '20px', padding: '2px 8px' }}>● Live</div>:<div style={{ fontSize: '10px', color: '#94a3b8', background: '#f0f2f5', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '2px 8px' }}>Idle</div>}
                  </div>
