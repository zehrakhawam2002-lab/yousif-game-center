...
export default function MenuPage() {
  ...
  const openAdd = () => { setEditItem(null); setForm({ name: '', category: 'drink', price: '' }); setShowModal(true) }
  const openEdit = (item) => { setEditItem(item); setForm({ name: item.name, category: item.category, price: item.price }); setShowModal(true) }
  ...
  const toggleAvailable = async (item) => { await supabase.from('menu_items').update({ is_available: !item.is_available }).eq('id', item.id); loadData() }
  const deleteItem = async (id) => { if (!confirm('Delete this item?')) return; await supabase.from('menu_items').delete().eq('id', id); loadData() }
          ...
          {loading ? <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>Loading...</div> : (
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                <thead><tr><th style={thStyle}>Item</th><th style={thStyle}>Category</th><th style={thStyle}>Price</th><th style={thStyle}>Status</th><th style={thStyle}>Actions</th></tr></thead>
                <tbody>
                  {items.length ? items.map(item => (
                    <tr key={item.id}>
                      <td style={tdStyle}><span style={{ fontSize: '18px', marginRight: '8px' }}>{ICONS[item.category] || '📦'}</span><strong>{item.name}</strong></td>
                      <td style={tdStyle}><span style={{ background: '#eef2ff', color: '#6366f1', padding: '2px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', textTransform: 'capitalize' }}>{item.category}</span></td>
                      <td style={{ ...tdStyle, fontWeight: '700', color: '#6366f1' }}>{fmtIQD(item.price)}</td>
                      <td style={tdStyle}><button onClick={() => toggleAvailable(item)} style={{ padding: '3px 10px', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '11px', fontWeight: '700', background: item.is_available ? '#d1fae5' : '#fee2e2', color: item.is_available ? '#065f46' : '#991b1b' }}>{item.is_available ? '✅ Available' : '❌ Unavailable'}</button></td>
                      <td style={tdStyle}><div style={{ display: 'flex', gap: '6px' }}><button onClick={() => openEdit(item)} style={{ padding: '4px 10px', border: '1px solid #e2e8f0', borderRadius: '7px', background: '#fff', cursor: 'pointer', fontSize: '12px', color: '#6366f1', fontWeight: '600' }}>✏️ Edit</button><button onClick={() => deleteItem(item.id)} style={{ padding: '4px 10px', border: '1px solid #fee2e2', borderRadius: '7px', background: '#fee2e2', cursor: 'pointer', fontSize: '12px', color: '#991b1b', fontWeight: '600' }}>🗑</button></div></td>
                    </tr>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '400px' }}>
            ...
            <div style={{ marginBottom: '1rem' }}><label style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '5px', display: 'block', textTransform: 'uppercase' }}>Name</label><input value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="e.g. Pepsi" style={{ width: '100%', padding: '9px 11px', border: '1px solid #cbd5e1', borderRadius: '9px', fontSize: '13px', boxSizing: 'border-box' }} /></div>
            <div style={{ marginBottom: '1rem' }}><label style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '5px', display: 'block', textTransform: 'uppercase' }}>Category</label><select value={form.category} onChange={e => setForm({...form, category: e.target.value})} style={{ width: '100%', padding: '9px 11px', border: '1px solid #cbd5e1', borderRadius: '9px', fontSize: '13px' }}>{CATS.map(c => <option key={c} value={c}>{ICONS[c]} {c.charAt(0).toUpperCase() + c.slice(1)}</option>)}</select></div>
            <div style={{ marginBottom: '1.25rem' }}><label style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '5px', display: 'block', textTransform: 'uppercase' }}>Price (IQD)</label><input type="number" value={form.price} onChange={e => setForm({...form, price: e.target.value})} placeholder="e.g. 1500" style={{ width: '100%', padding: '9px 11px', border: '1px solid #cbd5e1', borderRadius: '9px', fontSize: '13px', boxSizing: 'border-box' }} /></div>
