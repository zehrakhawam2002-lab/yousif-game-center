...
export default function PCPage() {
  ...
  const [settings, setSettings] = useState({ price_ps: 3000, currency: 'IQD' })
  ...
  const loadData = async () => {
    const { data: st } = await supabase.from('stations').select('*, sessions(id, started_at, customer_name, status)').eq('type', 'pc').eq('is_active', true).order('created_at')
  ...
  const addStation = async () => {
    if (!newName) return
    await supabase.from('stations').insert({ name: newName, type: 'pc' })
      ...
      <div style={{ marginLeft: '235px', flex: 1 }}>
        ...
        <div style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1e293b' }}>PC Stations 🖥️</h2>
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button onClick={() => setShowAddModal(true)} style={{ background: '#6366f1', color: '#fff', border: 'none', borderRadius: '8px', padding: '6px 14px', fontSize: '13px', cursor: 'pointer', fontWeight: '600' }}>+ Add PC</button>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px' }}>
            {stations.map(st => {
              const session = getActiveSession(st), elapsed = getElapsed(session), cost = session ? (elapsed/3600000)*settings.price_pc : 0
              return (
                <div key={st.id} style={{ background: '#fff', border: session ? '2px solid #10b981' : '1px solid #e2e8f0', borderRadius: '12px', padding: '1.1rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '10px', color: '#64748b', background: '#f0f2f5', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '2px 8px', display: 'inline-block', marginTop: '2px' }}>PC</div>
                    </div>
                    {session
                      ? <div style={{ fontSize: '10px', fontWeight: '700', color: '#065f46', background: '#d1fae5', borderRadius: '20px', padding: '2px 8px' }}>● Live</div>
                  ...
                  <div style={{ fontSize: session ? '28px' : '16px', fontWeight: session ? '800' : '400', color: session ? '#1e293b' : '#94a3b8', margin: '6px 0 2px', fontVariantNumeric: 'tabular-nums' }}>
                    {session ? fmtTimer(elapsed) : 'Idle'}
                  </div>
                  <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '10px' }}>
                    {fmtIQD(settings.price_pc)}/hr {session ? `• ${fmtIQD(cost)}` : ''}
                  </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '400px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <span style={{ fontSize: '15px', fontWeight: '700', color: '#1e293b' }}>Add PC Station</span>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '5px', display: 'block', textTransform: 'uppercase' }}>PC Name</label>
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. PC #4" style={{ width: '100%', padding: '9px 11px', border: '1px solid #cbd5e1', borderRadius: '9px', fontSize: '13px', boxSizing: 'border-box' }} />
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '400px' }}>
            ...
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '12px 14px', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '18px', fontWeight: '800', color: '#1e293b' }}>{fmtIQD(settings.price_pc)}<span style={{ fontSize: '12px', fontWeight: '400', color: '#64748b' }}>/hr</span></div>
              </div>
            </div>
            <div style={{ marginBottom: '1rem' }}>
